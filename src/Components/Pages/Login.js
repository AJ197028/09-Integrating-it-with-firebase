    ...
    const submitHandler = async (e) => {
        ...
        if (!ctx.login) {
            if (enteredEmail.current.value.length > 0 && enteredpass.current.value.length > 0 && enteredConfirmPass.current.value.length > 0) {
                if (enteredpass.current.value === enteredConfirmPass.current.value) {
                    let responce = await fetch(
                        'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyCFLf3xrXtLDmzGoNpwolS-93He6eIkTNY',
                        {
                            method: 'POST',
                            body: JSON.stringify({
            ...
            if (enteredEmail.current.value.length > 0 && enteredpass.current.value.length > 0) {
                let responce = await fetch(
                    'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyCFLf3xrXtLDmzGoNpwolS-93He6eIkTNY',
                    {
                        method: 'POST',
                        body: JSON.stringify({
                            email: enteredEmail.current.value,
